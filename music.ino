...
// This code uses PROGMEM to fit the melodyDoom to flash as it was to long to fit
// in SRAM. It may not work on other Arduino arquitectures other than AVR


const int melodyFather[] PROGMEM = {
...
// there are two values per note (pitch and duration), so for each note there are four bytes
 ...
 notes = sizeof(melodyFather) / sizeof(melodyFather[0]) / 2;
...
// iterate over the notes of the melodyDoom.
...
tempo = map (pot_deger, 0, 1023, 50, 400);
 wholenote = (60000 * 4) / tempo;
    ...
    tone(buzzer, pgm_read_word_near(melodyFather+thisNote), noteDuration * 0.9);
...
void Doom() {
  ...
  // sizeof gives the number of bytes, each int value is composed of two bytes (16 bits)
// there are two values per note (pitch and duration), so for each note there are four bytes
 notes = sizeof(melodyDoom) / sizeof(melodyDoom[0]) / 2;
...
tempo = map (pot_deger, 0, 1023, 50, 400);
 wholenote = (60000 * 4) / tempo;
    ...
    // we only play the note for 90% of the duration, leaving 10% as a pause
    tone(buzzer, pgm_read_word_near(melodyDoom+thisNote), noteDuration * 0.9);
